...
let error : Error.t -> string = function
  ...
  | `NotFound n -> Printf.sprintf "No var '%s' in scope" (Intern.lookup n)
  | `NoField (field, record) ->
    Printf.sprintf
      ...
      (String.concat ", "
       @@ List.map
            (fun (label, field) -> Printf.sprintf "%s: %s" (Intern.lookup label) field)
            record)
...
let record ?(tail = None) (prn : 'a -> string) (fields : (Ident.t * 'a) list) : string =
  let fields =
    List.map
      (fun (ident, v) -> Printf.sprintf "%s = %s" (Intern.lookup ident) (prn v))
      fields
    |> String.concat ","
  in
  match tail with
...
let rec pattern (prn : 'a -> string) : 'a pattern -> string = function
  | PCtor (ident, args) ->
    String.concat " " @@ (Intern.lookup ident :: List.map (pattern prn) args)
  | PLit lit -> literal prn lit
  | PRec fields -> record (pattern prn) fields
  | PVar v -> Intern.lookup v
  | PWild -> "_"
;;

let base (prn : 'a -> string) : 'a base -> string = function
  | `Var ident -> Intern.lookup ident
  ...
  | `Proj (term, field) -> Printf.sprintf "%s.%s" (prn term) (Intern.lookup field)
...
let rec value : value -> string = function
  | `Lam (x, body) ->
    ...
    let var = `Neutral (NVar (0, Intern.underscore)) in
    let body = value @@ body var in
    Printf.sprintf "\\%s -> %s" (Intern.lookup x) body
  | `Pi (x, dom, cod) ->
    print_endline "Pretty.value.Pi";
    let var = `Neutral (NVar (0, Intern.underscore)) in
    let cod = cod var in
    Printf.sprintf "(%s : %s) -> %s" (Intern.lookup x) (value dom) (value cod)
  | `Neutral n ->
...
and neutral : neutral -> string = function
  | NVar (n, ident) ->
    Printf.sprintf "NVar %s %s" (string_of_int n) (Ident.Intern.lookup ident)
  | NApp (f, x) -> Printf.sprintf "(%s %s)" (neutral f) (value x)
  | NMeta m -> Printf.sprintf "?%s" @@ string_of_int m
  | NProj (tm, field) -> Printf.sprintf "%s.%s" (neutral tm) (Ident.Intern.lookup field)
;;

let term_binders (prn : 'a -> string) : 'a term_binders -> string = function
  | `Lam (x, body) -> Printf.sprintf "\\%s -> %s" (Intern.lookup x) (prn body)
  | `Pi (x, dom, cod) ->
    Printf.sprintf "(%s : %s) -> %s" (Intern.lookup x) (prn dom) (prn cod)
  ...
  | `Let (ident, None, expr, body) ->
    Printf.sprintf "let %s = %s in\n%s" (Intern.lookup ident) (prn expr) (prn body)
;;
...
let declaration (prn : 'a -> string) : 'a declaration -> string = function
  | Function { ident; typ; body } ->
    Printf.sprintf "def %s (%s) do\n%s\n\n" (Intern.lookup ident) (prn typ) (prn body)
  ...
  | RecordDecl { ident; params; fields } ->
    let params =
      ...
      @@ List.map
           (fun (ident, ty) -> Printf.sprintf "%s : %s" (Intern.lookup ident) (prn ty))
           params
    in
    let fields =
      String.concat "\n\t"
      @@ List.map
           (fun (field, ty) -> Printf.sprintf "%s : %s" (Intern.lookup field) (prn ty))
           fields
    in
    Printf.sprintf "data %s %s where\n\t%s\n\n" (Intern.lookup ident) params fields
;;
