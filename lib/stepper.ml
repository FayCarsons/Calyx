...
let default_node_state =
  { expanded = true; focus_visible = true; context_visible = false }
;;
...
type tui_state =
  ...
  ; node_states : node_ui_state PathMap.t
  ; scroll_offset : int
  ; viewport_height : int
  }
...
let get_node_state state path =
  Map.find state.node_states path |> Option.value ~default:default_node_state
;;

let set_node_state state path node_state =
  { state with node_states = Map.set state.node_states ~key:path ~data:node_state }
;;

let get_node_at_path root path =
...
let format_location (loc : Pos.t) = Printf.sprintf "%d:%d" loc.lo.line loc.lo.column
...
let flatten_visible state =
  let rec go ~path ~depth ~ancestors_last ~is_last node acc =
    ...
    let acc = line :: acc in
    let node_state = get_node_state state path in
    ...
    then (
      ...
      in
      (* TODO: Add Vector.fold_lefti to vector.ml to avoid this conversion *)
...
let render_line state line =
  ...
  in
  let focus_indicator = if node_state.focus_visible then "v" else ">" in
  ...
  I.hcat
    ...
    ; I.string
        A.(base_attr ++ fg (gray 12))
        (Printf.sprintf "[%s%s]" tree_indicator focus_indicator)
    ; I.string base_attr " "
...
let render_focus_for_node state line =
  let node_state = get_node_state state line.path in
  if not node_state.focus_visible
  then []
  else (
    let focus_s =
    ...
    List.map lines ~f:(fun l -> I.string A.(fg white) (indent ^ l)))
;;
...
let render_context_for_node state line =
  ...
  then I.empty
    ...
    then I.string A.(fg (gray 10)) (indent ^ "(empty context)")
    else
      I.vcat
        (List.map ctx ~f:(fun (name, value, typ_opt) ->
           let value_s = Sexp.to_string (Term.sexp_of_value value) in
           let typ_s =
             match typ_opt with
             | Some t -> " : " ^ Sexp.to_string (Term.sexp_of_value t)
             | None -> ""
           in
           I.string A.(fg (gray 12)) (indent ^ name ^ " = " ^ value_s ^ typ_s))))
;;

let node_render_height state line =
  let node_state = get_node_state state line.path in
  let focus_height =
    if node_state.focus_visible
    then (
      let focus_s =
        match line.node.data with
        | Trace.Pending j -> format_focus j.stage j.focus
  ...
  let ctx_height =
    ...
    then (
      let ctx =
        ...
        | Trace.Complete (j, _) -> Lazy.force j.context
      in
      if List.is_empty ctx then 1 else List.length ctx)
    else 0
  in
  1 + focus_height + ctx_height
;;

let render_help state =
  let mode_str =
    match state.step_mode with
    | Trace.Into -> "INTO"
    | Trace.Over -> "OVER"
    | Trace.Out -> "OUT"
    | Trace.Run -> "RUN"
...
let render state =
  ...
  let line_images =
    List.concat_map visible_lines ~f:(fun line ->
      ...
      [ main ] @ focus @ (if I.height ctx > 0 then [ ctx ] else []))
  in
  let scroll_info =
...
let move_cursor_right state =
  ...
  | None -> state
  | Some node ->
    let node_state = get_node_state state state.cursor in
...
let toggle_expand state =
  let node_state = get_node_state state state.cursor in
  set_node_state state state.cursor { node_state with expanded = not node_state.expanded }
;;
...
let toggle_focus state =
  ...
  set_node_state
    state
    state.cursor
    { node_state with focus_visible = not node_state.focus_visible }
;;

let toggle_context state =
  let node_state = get_node_state state state.cursor in
  set_node_state
    state
    state.cursor
...
let jump_to_top state = { state with cursor = [ 0 ]; scroll_offset = 0 }

let jump_to_bottom state =
  let lines = flatten_visible state in
...
let apply_nav_action state = function
  ...
  | ToggleFocus -> toggle_focus state
...
let tui_state : tui_state ref = ref initial_state
...
let on_start () =
  ...
  tui_state := { initial_state with root = Some (make_virtual_root ()) };
...
let render_and_get_action () : Trace.step_action =
  ...
  | Some t ->
    let rec loop () =
      ...
      tui_state := ensure_cursor_visible !tui_state;
      let img = render !tui_state in
      Notty_unix.Term.image t img;
      match Notty_unix.Term.event t with
      | `Resize _ -> loop ()
      | event ->
        (match key_to_action event with
         | None -> loop ()
         | Some (Nav nav) ->
           tui_state := apply_nav_action !tui_state nav;
           loop ()
         | Some (Step StepInto) ->
...
let run ~f = Trace.handle_interactive ~on_enter ~on_leave ~on_start ~on_finish ~f
